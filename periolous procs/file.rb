...
def some?(arr, &prc)
    output = false
    arr.each {|ele| if prc.call(ele) then output = true end}
    output
end
...
def exactly?(arr, n, &prc)
    count = 0
    arr.each {|ele| if prc.call(ele) == true then count += 1 end}
    if count == n then true else false end
end
...
def at_least?(arr, n, &prc)
    true_count = 0
    arr.each {|ele| if prc.call(ele) == true then true_count += 1 end}
    if true_count >= n then true else false end
end
...
def every?(arr, &prc)
    canary = true
    arr.each {|ele| if prc.call(ele) == false then canary = false end}
    canary
end
...
def at_most?(arr, n, &prc)
    canary = 0
    arr.each {|ele| if prc.call(ele) == true then canary += 1 end}
    if canary <= n then true else false end
end
...
def first_index(arr, &prc)
    ...
    if indices == [] then nil else indices.min end
end
...
def xnor_select(arr, prc1, prc2)
    ...
    #return new array if both prcs true or both false
end
...
def filter_out!(arr, &prc)
    ...
    arr
    #remove elements of input array if prc.call(ele) == true
end
...
def multi_map(arr, n=1, &prc)
    # return new array where elements are run through prc n times, default = 1
    i = 0
    arr.each do |ele|
        (1..n).each do
